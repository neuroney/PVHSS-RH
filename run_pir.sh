...
for db_size in "${SIZE_ARRAY[@]}"; do
    ...
    if enabled "$RUN_GROUP_DC"; then
        ...
        echo "pir_group_dc,$db_size,$record_bits,$SEED,$setup,$offline,$query,$answer_total,$verify_decode,$online,$total_bits,$correctness,$tamper_rejected" >> "$COMPARE_OUT_FILE"
        echo "done"
    fi

    if enabled "$RUN_GROUP_OT"; then
        echo -n "  pir_group_ot db_size=$db_size ... "
        ...
        echo "pir_group_ot,$db_size,$logn,$record_bits,$index,$SEED,$setup,$setup_vhss,$setup_extra,$gen,$query,$answer0,$answer0_eval,$answer0_proof,$answer1,$answer1_eval,$answer1_proof,$verify,$decode,$correctness,$tamper_rejected,$query_bits,$answer0_bits,$answer1_bits,$total_bits,$formula_query_bits,$formula_answer_bits,$formula_total_bits" >> "$GROUP_OT_OUT_FILE"

        offline=$(sum_ms "$setup" "$gen")
        answer_total=$(sum_ms "$answer0" "$answer1")
        verify_decode=$(sum_ms "$verify" "$decode")
        online=$(sum_ms "$query" "$answer0" "$answer1" "$verify" "$decode")
        echo "pir_group_ot,$db_size,$record_bits,$SEED,$setup,$offline,$query,$answer_total,$verify_decode,$online,$total_bits,$correctness,$tamper_rejected" >> "$COMPARE_OUT_FILE"
        echo "done"
    fi
