...
// Platform detection: enable x86 BMI2/ADX intrinsics only on x86_64 with
// compiler-reported BMI2 support. On all other platforms (ARM64, etc.),
// portable C++17 fallbacks are used instead.
#if defined(__x86_64__) && defined(__BMI2__)
  #define LHSS_USE_X86_INTRINSICS 1
  #include <x86intrin.h>
#else
  #define LHSS_USE_X86_INTRINSICS 0
#endif
...
{
#if LHSS_USE_X86_INTRINSICS
  // x86 BMI2 path: MULX instruction — no flag dependency, pipelined.
  low = _mulx_u64(a, b , reinterpret_cast<unsigned long long int*>(&high));
#else
  // Portable path: use 128-bit multiplication (GCC/Clang on ARM64/aarch64).
  u128 prod = static_cast<u128>(a) * b;
  low  = static_cast<std::uint64_t>(prod);
  high = static_cast<std::uint64_t>(prod >> 64);
#endif
}
...
#else
  u128 prod = static_cast<u128>(a) * b;
  mul[0] = static_cast<std::uint64_t>(prod);
  mul[1] = static_cast<std::uint64_t>(prod >> 64);
#endif
}
...
#else
  mul = static_cast<std::uint64_t>((static_cast<u128>(a) * b) >> 64);
#endif
}
...
#else
  // Portable carry detection: if a + b wraps, the result is less than a.
  add = a + b;
  return (add < a) ? 1 : 0;
#endif
}
...
#else
  // Portable 128-bit addition: add low words, propagate carry to high words.
  res[0] = a[0] + b[0];
  unsigned char carry = (res[0] < a[0]) ? 1 : 0;
  res[1] = a[1] + b[1] + carry;
#endif
}
...
#else
  // Portable in-place 128-bit addition.
  unsigned char carry = (res[0] + a[0] < res[0]) ? 1 : 0;
  res[0] += a[0];
  res[1] += a[1] + carry;
#endif
}
