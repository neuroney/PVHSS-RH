...
struct PolyConv
{
  ...
  static void CRTToBigPoly(const CRTPoly& crt_p, BigPoly& p_dest)
  {
    ...
    for (std::size_t j = 0; j < Params::n; ++j)
    {
      for (std::size_t i = 0; i < Params::num_moduli; ++i)
      {
        rem = static_cast<unsigned long>(crt_p.small_polys[i].coeffs[j]);
        rem *= static_cast<unsigned long>(Params::crt_q.q_div_qi_invs_mod_qis[i]);
  ...
  static void BigToCRTPoly(const BigPoly& bp, CRTPoly& p_dest)
  {
    ...
    {
      for (std::size_t i = 0; i < Params::num_moduli; ++i)
        p_dest.small_polys[i].coeffs[j] = mpz_mod_ui(fake.get_mpz_t(), bp.GetConstVal(j).get_mpz_t(), static_cast<unsigned long>(Params::z_qi[i].modulus));
    }
  }
  ...
  static void BigToSmallPoly(const BigPoly& bp, Poly& p_dest)
  {
    ...
    mpz_class fake;
    for (std::size_t j = 0; j < Params::n; ++j)
      p_dest.coeffs[j] = mpz_mod_ui(fake.get_mpz_t(), bp.GetConstVal(j).get_mpz_t(), static_cast<unsigned long>(Params::z_qi[0].modulus));
  }
};
} // namespace
