...
struct CRT
{
  ...
  void Reconstruct(const std::vector<UIntType>& rems, mpz_class& val)
  {
    ...
    mpz_class rem = static_cast<unsigned long>(rems[0]);
    rem *= static_cast<unsigned long>(q_div_qi_invs_mod_qis[0]);
    rem *= q_div_qis[0];
    val += rem;
    for (std::size_t i = 1; i < nummod; ++i)
    {
      rem = static_cast<unsigned long>(rems[i]);
      rem *= static_cast<unsigned long>(q_div_qi_invs_mod_qis[i]);
      rem *= q_div_qis[i];
      val += rem;
    }
    val %= q;
  ...
  void ReconstructWithIntermediateModReduce(const std::vector<UIntType>& rems, mpz_class& val)
  {
    ...
    for (std::size_t i = 1; i < nummod; ++i)
    {
      rem = static_cast<unsigned long>(rems[i]);
      rem *= static_cast<unsigned long>(q_div_qi_invs_mod_qis[i]);
      rem %= static_cast<unsigned long>(qis[i]);
      rem *= q_div_qis[i];
      val += rem;
    }
    val %= q;
 ...
 private:
  ...
  void PreComputeQandQdivQis()
  {
    q = 1;
    for (std::size_t i = 0; i < nummod; ++i)
      q *= static_cast<unsigned long>(qis[i]);

    for (std::size_t i = 0; i < nummod; ++i)
      q_div_qis.push_back(q/static_cast<unsigned long>(qis[i]));
  }
};
}
