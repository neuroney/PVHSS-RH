...
struct Params
{
  ...
  static void ComputeQDivP()
  {
    if (p.size() ==  1)
    {
      P = static_cast<unsigned long>(p[0]);
    }
    else
    {
      CRT crt_p(p);
  ...
  static void ShowParams()
  {
    ...
    for (std::size_t i = 0; i < num_moduli; ++i)
    {
      printf(" \n\t(%llu,", (unsigned long long)z_qi[i].modulus);
      // printf(" %lu,", nth_roots_of_unities[i]);
      // printf(" %lu,", inv_nth_roots_of_unities[i]);
      printf(" %llu,", (unsigned long long)mth_roots_of_unities[i]);
      printf(" %llu)", (unsigned long long)inv_mth_roots_of_unities[i]);
  ...
  static void ComputeQ(const std::vector<UIntType>& ctxt_moduli)
  {
    ...
    {
      Q *= static_cast<unsigned long>(qi);
  ...
  static void ComputeP(const std::vector<UIntType>& plain_moduli)
  {
    ...
    {
      p.push_back(plain);
      P *= static_cast<unsigned long>(plain);
    }
  }
