...
void PVHSS_Add(vec_ZZ_pX &tb, vec_ZZ_pX &tb1, vec_ZZ_pX &tb2)
{
    tb.SetLength(2);
    tb[0] = tb1[0] + tb2[0];
    tb[1] = tb1[1] + tb2[1];
...
void PVHSS_Eval(ZZ_pX &tby, ep_t g1T1, ep2_t g2T2, int b, PVHSS_Para pvhssPara, PKE_Para pkePara,
                ...
                Vec<vec_ZZ_pX> C_X, Vec<vec_ZZ_pX> PRF, vector<vector<int>> F_TEST, int degree_f, 
                vec_ZZ_pX C_1)
{
    vec_ZZ_pX tmp1, tmp2;
    tmp1.SetLength(2);
    ...
    PVHSS_Mult(dp_prev[0], pkePara, modulus, ek, C_1);
    ...
    for (int i = 1; i <= k; i++)
    {
        for (int s = 0; s <= degree_f; s++)
        {
            ...
            dp_curr[s][0] = 0;
            dp_curr[s][1] = 0;
            PVHSS_Add(dp_curr[s], dp_curr[s], dp_prev[s]);
            for (int j = 1; j <= s; j++)
            {
                copy(begin(dp_prev[s - j]), end(dp_prev[s - j]), begin(tmp1));
                for (int h = 0; h < j; ++h)
                {
                    PVHSS_Mult(tmp2, pkePara, modulus, tmp1, C_X[i - 1]);
                    copy(begin(tmp2), end(tmp2), begin(tmp1));
                }
                PVHSS_Add(dp_curr[s], dp_curr[s], tmp1);
            }
        }
    ...
    {
        PVHSS_Add(y_b_res, y_b_res, dp_prev[s]);
    }
    ...
    ZZ Tb;
    PVHSS_Mult(y_b_res, pkePara, modulus, y_b_res, C_alpha);
    conv(tb0, y_b_res[0]);
    eval_ZZX(Tb, tb0);
    if (b == 1)
    {
